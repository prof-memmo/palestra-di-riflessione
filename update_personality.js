...
// Create a mock window object so exercises.js can load
global.window = {};

// Load the current data
const data = require('./js/exercises.js');

const acc = data.accoglienza;

if (!acc) {
    console.error("accoglienza not found!");
    process.exit(1);
}

// Modify the specific cards
acc.forEach(ex => {
    if (ex.id === 10007) {
        ...
        if (ex.questions) ex.questions.forEach(q => delete q.answer);
    }
    else if (ex.id === 10008) {
        ex.type = "personality-test";
...
const finalContent = 'window.exercisesData = ' + jsonStr + ';\n\nif (typeof module !== \'undefined\') module.exports = window.exercisesData;\n';

fs.writeFileSync('./js/exercises.js', finalContent, 'utf8');
console.log('Successfully updated js/exercises.js');
